...
def _get_reward_model(base_pretrained_model, base_llm_model, value_head_prefix="score", packing_samples=False):
    class RewardModel(base_pretrained_model):
        ...
        def __init__(self, config: AutoConfig):
            ...
            # Add value head for only response
            setattr(self, f"{value_head_prefix}_only", nn.Linear(config.hidden_size, 1, bias=False))
        ...
        def forward(
            ...
            # Get rewards from both heads
            values = getattr(self, self.value_head_prefix)(last_hidden_states).squeeze(-1)
            values_only = getattr(self, f"{self.value_head_prefix}_only")(last_hidden_states).squeeze(-1)

            if self.packing_samples:
                values = gather_and_pad_tensor(values, ring_attn_group, ring_attn_pad_len, indices, batch, seqlen)
                values_only = gather_and_pad_tensor(values_only, ring_attn_group, ring_attn_pad_len, indices, batch, seqlen)

            reward = values.gather(dim=1, index=eos_indices).squeeze(1)
            reward_only = values_only.gather(dim=1, index=eos_indices).squeeze(1)

            if not self.training and self.normalize_reward:
                reward = (reward - self.mean) / self.std
                reward_only = (reward_only - self.mean) / self.std

            if return_output:
                return reward, reward_only, outputs
            else:
                return reward, reward_only
